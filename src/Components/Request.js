 ...
 class Request extends Component {
    ...
    editBook(event)
    {
      ...
      else
      {
        ...
        .ref('Books')
        .child(this.state.id+"/assignedto")
        .push({
        ...
        console.log(ass);
        ass = Object.assign({[ref.key]:{status:"requested",userid:firebase.auth().currentUser.email}})
        console.log(ass);
        this.setState({
          assignedto:ass
        })
     ...
     render() {
         return (
             ...
             <br/>
                   ...
                   {this.state.SearchBooks.map((book,flag = false) => (<>

                    {book.assignedto.map(present => present ?  
                    book.assignedto.map(assbook =>   Object.values(assbook).map(x => (x.userid === firebase.auth().currentUser.email)?
                    flag = true:
                    null)):
                    flag = false)}
                    ...
                    {flag !== true ? <div>
                            ...
                            <Button variant="primary" onClick={() => this.flagedit(true,book)}>Add Book</Button>

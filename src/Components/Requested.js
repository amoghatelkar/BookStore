     ...
     render() {
         ...
         return (
             ...
             <br/>
                   ...
                   <>  
                   {book.assignedto.map(present => present ?  book.assignedto.map(assbook =>   Object.values(assbook).map(x => (x.userid === firebase.auth().currentUser.email && x.status === "requested")?
                                 <div>
                <Card key = {book.id}>
                <Card.Header as="h5">{book.bookname}</Card.Header>
                <Card.Body>
